...
func getPlaylistsWithFavorite(ctx context.Context, db connOrTx) ([]PlaylistWithFavoriteRow, error) {
	...
	if err := db.SelectContext(
		ctx,
		&allPlaylists,
		"SELECT id,ulid,name,user_account,is_public,created_at,updated_at,count FROM playlist_favorite_count INNER JOIN playlist on playlist_favorite_count.playlist_id = playlist.id where playlist.is_public ORDER BY count DESC LIMIT 150",
	); err != nil {
		return nil, fmt.Errorf(
			"error Select playlist by is_public=true: %w",
			err,
...
func getPlaylistDetailByPlaylistULID(ctx context.Context, db connOrTx, playlistULID string, viewerUserAccount *string) (*PlaylistDetail, error) {
	...
	songRows := make([]SongRow, 0, len(resPlaylistSongs))
...
func insertPlaylistFavorite(ctx context.Context, db connOrTx, playlistID int, favoriteUserAccount string, createdAt time.Time) error {
	if _, err := db.ExecContext(
		...
		return fmt.Errorf(
			"error Insert playlist_song by playlist_id=%d, favorite_user_account=%s, created_at=%s: %w",
...
func apiPlaylistsHandler(c echo.Context) error {
	...
	_account, _ := sess.Values["user_account"]
	userAccount := _account.(string)
...
func initializeHandler(c echo.Context) error {
	userLastCreatedAt := "2022-04-24 08:11:18.000"
	playlistLastId := 393165
	playlistFavoriteLastCreatedAt := "2022-04-24 08:49:59.000"
	ctx := c.Request().Context()
